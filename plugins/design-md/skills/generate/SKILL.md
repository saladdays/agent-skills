...
- **存在しない** → Step 1 へ進む
- **存在する** → 既存の DESIGN.md を Read で読み、内容を把握した上でユーザーに確認する:
  「既存の DESIGN.md があります。内容を確認しました。更新しますか？ それとも新しく作り直しますか？」
  - 更新 → 既存の内容をベースに、変更したい箇所をヒアリング
...
**フォーマット:**
- `resources/format-spec.md` の構成に厳密に従う
...
- [ ] 推奨行数の範囲内か
- [ ] Density Guide を含むすべての対応表に Reasoning 列があるか
...
3. DESIGN.md は「生きたドキュメント」です。プロダクトの成長に合わせて更新していきましょう
```
...
- `resources/format-spec.md` — フォーマット仕様
