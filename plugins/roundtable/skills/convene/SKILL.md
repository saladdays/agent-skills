...
- **Quick**: Phase 2 スキップ。参照ファイルを読まない。SKILL.md 本体で完結
  **エスカレーション**: Phase 1 の結果で重大な対立（専門家の総合評価が2段階以上乖離、または推奨アクションが正反対）が2つ以上ある場合、Standard への昇格をユーザーに提案
...
**モデル混合（利用可能な場合に推奨）**: 異なるモデルを専門家ごとに割り当てることで、
同一モデルの擬似的多様性問題を緩和できる。MAD 研究で最も安定した効果が確認されている手法。
- **Cursor**: サブエージェントの `model` フィールドで異なるプロバイダーを指定可能
  （例: 専門家A = Claude, 専門家B = GPT-4o, DA = Gemini）
- **Claude Code**: Agent tool の `model` パラメータで Opus / Sonnet / Haiku を使い分け可能
...
**サブエージェント利用不可（fallback）**: 各専門家を逐次実行する。
...
**要約の対称性チェック**（オーケストレーターの要約バイアスを緩和する）：
...
1. **Step 1（事前コミット）**: 統合結果を見せる**前に**、各エージェントに
   「Phase 1 でのあなたの結論は今も変わらないか？」を確認する
2. **Step 2（差分検出）**: 統合結果の要約を見せ、
   「あなたの結論と統合結果に差分がある場合、指摘してください」と問う

Step 1 で「変わらない」と答えたのに Step 2 で意見を変えた場合、
それは sycophancy の兆候であり、元の意見を尊重する。
